...
BEGIN_MESSAGE_MAP(CN64SoundListToolDlg, CDialog)
    ...
    ON_BN_CLICKED(IDC_MASSIMPORT, &CN64SoundListToolDlg::OnBnClickedMassimport)
END_MESSAGE_MAP()
...
void CN64SoundListToolDlg::OnBnClickedMassimport()
{
    if (alBankCurrent == NULL) return;
    if (percussionMode != NORMAL) return;

    FILE* originalFile = NULL;
    FILE* outFile = NULL;

    uint8_t* originalUnkValues = NULL;
    uint32_t* pointers = NULL;
    uint8_t* encodedData = NULL;

    // File / folder dialogs

    static TCHAR BASED_CODE szFilter[] = _T("Voice Data (*.bin)|*.bin|") _T("All Files (*.*)|*.*||");

    CFolderPickerDialog folderDlg(NULL, 0, this);
    folderDlg.m_ofn.lpstrTitle = _T("Select the folder containing the WAV files...");
    INT_PTR result = folderDlg.DoModal();
    if (result != IDOK) return;

    CString wavsPath = folderDlg.GetPathName();

    CFileDialog fileDlg(TRUE, NULL, _T("VOICEData.bin"), OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, szFilter);
    fileDlg.m_ofn.lpstrTitle = _T("Select the original VOICEData.bin file...");
    result = fileDlg.DoModal();
    if (result != IDOK) return;

    CString originalVoiceDataPath = fileDlg.GetPathName();

    CFileDialog fileDlg2(FALSE, NULL, _T("VOICEDataNew.bin"), OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, szFilter);
    fileDlg2.m_ofn.lpstrTitle = _T("Select the new VOICEData.bin file to insert the audio files into...");
    result = fileDlg2.DoModal();
    if (result != IDOK) return;

    CString outputPath = fileDlg2.GetPathName();

    // Read pointers of the original VOICEData.bin file

    originalFile = fopen(originalVoiceDataPath, "rb");
    if (originalFile == NULL)
    {
        MessageBox("Error opening original VOICEData file.", LoadResourceText(IDS_STRINGERROR));
        goto cleanup;
    }

    originalUnkValues = new uint8_t[numberResults];
    fseek(originalFile, 4, SEEK_SET);

    for (int b = 0; b < numberResults; b++)
    {
        int value = fgetc(originalFile);
        originalUnkValues[b] = (uint8_t)value;
        fseek(originalFile, 3, SEEK_CUR);
    }

    fclose(originalFile);
    originalFile = NULL;

    // Start writing new VOICEData.

    outFile = fopen(outputPath, "wb");
    if (outFile == NULL)
    {
        MessageBox("Error opening output file.", LoadResourceText(IDS_STRINGERROR));
        goto cleanup;
    }

    pointers = new uint32_t[numberResults];
    encodedData = new uint8_t[0x100000];
    int pointerIndex = 0;

    fseek(outFile, numberResults * 4 + 4, SEEK_SET);

    for (int b = 0; b < numberResults; b++)
    {
        for (int i = 0; i < results[b].bank->count; i++)
        {
            for (int s = 0; s < results[b].bank->inst[i]->soundCount; s++)
            {
                ...
                if (!CN64AIFCAudio::ReadWavData(tempExportNameStr, wavPCMData, rawLength, samplingRate, hasLoopData, keyBase, loopStart, loopEnd, loopCount))
                {
                    goto cleanup;
                }
                ...
                memset(encodedData, 0, 0x100000);
                int outputBufferSize = 0;

                CMORTDecoder mortDecoder;
                mortDecoder.Encode(wavPCMData, rawLength, encodedData, outputBufferSize);

                pointers[pointerIndex] = ftell(outFile);
                pointerIndex++;

                fwrite(encodedData, 1, outputBufferSize, outFile);
    ...
    rewind(outFile);
    fputc((numberResults >> 24) & 0xFF, outFile);
    fputc((numberResults >> 16) & 0xFF, outFile);
    fputc((numberResults >> 8) & 0xFF, outFile);
    fputc((numberResults >> 0) & 0xFF, outFile);

    for (int b = 0; b < numberResults; b++)
    {
        fputc(originalUnkValues[b], outFile);
        fputc((pointers[b] >> 16) & 0xFF, outFile);
        fputc((pointers[b] >> 8) & 0xFF, outFile);
        fputc((pointers[b] >> 0) & 0xFF, outFile);
    }

    MessageBox("Process has finished successfully!");
...
cleanup:
    ...
    if (originalUnkValues) delete[] originalUnkValues;
